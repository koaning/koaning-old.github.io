...
function() {
    ...
    for (a = 0; a < d; a++) {
        ...
        if (u !== l[a][1]) {
            ...
            if (g === 0 && y === 0) continue;
            w = -m - b, E = m * b - g * y, S = w * w - 4 * E, S >= 0 ? (w < 0 ? x = -0.5 * (w - A(S)) : x = -0.5 * (w + A(S)), k = (m - x) * (m - x) + g * g, L = y * y + (b - x) * (b - x), k > L ? (k = A(k), N = (m - x) / k, C = g / k) : (L = A(L), N = y / L, C = (b - x) / L), p = new s([
                [C, -N],
                [N, C]
...
function() {
    ...
    for (U = w; U < s; ++U) {
        ...
        } else o[v - 1] >= 0 ? y = !1 : y = !0; if (y) return {
            solution: g,
            message: "Unbounded",
            iterations: U
        }
